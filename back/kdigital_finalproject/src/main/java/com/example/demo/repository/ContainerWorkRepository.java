...
public interface ContainerWorkRepository extends JpaRepository<ContainerWork, Integer> {
	...
	@Query(value = "SELECT container, timeEnd, block2, bay2, row2, tier2, crane "
			...
			+ "  GROUP BY container"
			+ ")"
			+ "AND row2 != 0 AND tier2 != 0")
	List<Object[]> findCurrentWorkAll();
	...
	// 크레인별로 마지막으로 일한 timeEnd, block2, bay2, row2, tier2 를 출력하는 쿼리
	@Query(value = "SELECT c.crane, c.max_time_end, cw.block2, cw.bay2, cw.row2, cw.tier2 " +
		       "FROM ( " +
		       "    SELECT crane, MAX(time_end) AS max_time_end " +
		       "    FROM container_work " +
